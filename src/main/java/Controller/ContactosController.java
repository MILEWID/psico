...
public class ContactosController {
     @Autowired
	ContactosService paciente ;
	
	@GetMapping()
	public ArrayList<ContactosEntity> obtenerContacto(){
		return paciente.obtenerContacto();
	}
	
	@PostMapping()
		public ContactosEntity guardarContacto(@RequestBody ContactosEntity paciente) {
			return this.paciente.guardarContacto(paciente);
	}
        @DeleteMapping(path ="/{id}")
                  public String eliminarContacto(@PathVariable("id") Long id){
                      boolean ok = this.paciente.eliminarContacto(id);
            if(ok){
